...
//                turretRelSetpoint = Math.max(Math.min(wrapAngle(desiredAngle - s_drivetrain.getHeading()), 90 - s_drivetrain.getHeading()), -90 - s_drivetrain.getHeading());
                output = Math.max(Math.min(turretController.calculate(getRobotTurretAngle(), desiredAngle), Constants.turretConstants.turretConfigs.maxSpeed), -Constants.turretConstants.turretConfigs.maxSpeed);
            }
